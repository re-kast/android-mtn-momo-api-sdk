...
```bash
cd android
./gradlew :momo-api-sdk:publishAndroidPublicationToLocalStagingRepository \
  -PVERSION_NAME="1.2.3"
```
...
This module is published as a Kotlin Multiplatform library, so it exposes one publication per target — `android`, `jvm`, and `kotlinMultiplatform`. To stage all of them at once, use `publishAllPublicationsToLocalStagingRepository`.
