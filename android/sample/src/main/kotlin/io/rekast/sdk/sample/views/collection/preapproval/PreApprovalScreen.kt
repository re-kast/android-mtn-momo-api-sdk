...
/**
 * Renders the Collection Pre-Approval screen: a create form plus check-status and cancel actions,
 * with a console panel showing each operation's outcome.
 *
 ...
 */
@Composable
fun PreApprovalScreen(navController: NavController?, snackStateFlow: SharedFlow<SnackBarComponentConfiguration>, showProgressBar: Boolean = false, viewModel: PreApprovalScreenViewModel?) {
    MomoScaffold(titleRes = R.string.pre_approval_screen, navController = navController, snackStateFlow = snackStateFlow) {
        if (showProgressBar || viewModel == null) {
            CircularProgressBarComponent()
        } else {
            val payerMsisdn by viewModel.payerMsisdn.observeAsState(Constants.EMPTY_STRING)
            val payerCurrency by viewModel.payerCurrency.observeAsState(Constants.SANDBOX_CURRENCY)
            val validityTime by viewModel.validityTime.observeAsState(Constants.EMPTY_STRING)
            ...
            LazyColumn(
                ...
                item {
                    MomoCard {
                        CardTitle(title = stringResource(id = R.string.pre_approval_title))
                        ...
                        LabeledField(stringResource(R.string.label_currency), payerCurrency, viewModel::onPayerCurrencyChanged)
                        LabeledField(stringResource(R.string.label_validity_minutes), validityTime, viewModel::onValidityTimeChanged, keyboardType = KeyboardType.Number)
                        LabeledField(stringResource(R.string.label_message), payerMessage, viewModel::onPayerMessageChanged)
                        Spacer(modifier = Modifier.height(12.dp))
                        OperationActionButton(
                            text = stringResource(R.string.action_create),
                            onClick = viewModel::createPreApproval,
                            enabled = payerMsisdn.isNotBlank() && validityTime.isNotBlank()
                        )
                        Spacer(modifier = Modifier.height(8.dp))
                        Row(modifier = Modifier.padding(top = 4.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            OperationActionButton(
                                ...
                                onClick = viewModel::cancelPreApproval,
                                enabled = !referenceId.isNullOrBlank(),
                                modifier = Modifier.weight(1f)
                            )
                        }
