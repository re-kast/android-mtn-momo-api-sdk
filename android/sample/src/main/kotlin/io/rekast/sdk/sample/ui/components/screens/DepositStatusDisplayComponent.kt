...
/**
 * Renders a read-only summary of a Disbursements deposit status inside a card: amount, currency,
 * financial transaction ID, external ID, payee, message, note, status, and failure reason. Fields
 * that are null or blank are omitted automatically.
 *
 * @param modifier Modifier applied to the root container.
 * @param title Card title displayed above the status details.
 * @param status LiveData holding the [DepositStatus] to display.
 */
@Composable
fun DepositStatusDisplayComponent(modifier: Modifier = Modifier, title: String, status: MutableLiveData<DepositStatus?>) {
...
@Composable
fun DepositStatusDisplayComponentPreview() {
