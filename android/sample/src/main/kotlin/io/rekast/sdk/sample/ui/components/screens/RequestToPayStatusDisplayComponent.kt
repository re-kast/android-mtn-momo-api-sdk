...
/**
 * Renders a read-only summary of a Collection request-to-pay status inside a card: amount, currency,
 * financial transaction ID, external ID, payer, message, note, status, and failure reason. Fields
 * that are null or blank are omitted automatically.
 *
 * @param modifier Modifier applied to the root container.
 * @param title Card title displayed above the status details.
 * @param status LiveData holding the [RequestToPayStatus] to display.
 */
@Composable
fun RequestToPayStatusDisplayComponent(modifier: Modifier = Modifier, title: String, status: MutableLiveData<RequestToPayStatus?>) {
...
@Composable
fun RequestToPayStatusDisplayComponentPreview() {
