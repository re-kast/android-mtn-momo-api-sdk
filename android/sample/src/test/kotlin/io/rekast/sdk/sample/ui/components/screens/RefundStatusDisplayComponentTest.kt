...
/**
 * Compose UI tests for [RefundStatusDisplayComponent], run on the JVM via Robolectric.
 *
...
@Config(sdk = [34])
class RefundStatusDisplayComponentTest {

    @get:Rule
    val composeRule = createComposeRule()

    private fun setDisplay(status: RefundStatus?) {
    ...
    fun `renders title with no status`() {
        setDisplay(null)
        composeRule.onNodeWithText("Refund Status").assertIsDisplayed()
