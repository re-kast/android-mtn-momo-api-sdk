...
/**
 * Compose UI tests for [DepositStatusDisplayComponent], run on the JVM via Robolectric.
 *
...
@Config(sdk = [34])
class DepositStatusDisplayComponentTest {

    @get:Rule
    val composeRule = createComposeRule()

    private fun setDisplay(status: DepositStatus?) {
    ...
    fun `renders title with no status`() {
        setDisplay(null)
        composeRule.onNodeWithText("Deposit Status").assertIsDisplayed()
