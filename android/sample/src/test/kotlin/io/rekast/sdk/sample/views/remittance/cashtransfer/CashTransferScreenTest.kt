...
/** Compose UI tests for the Remittance Cash Transfer screen ([CashTransferScreen]), via Robolectric. */
@RunWith(RobolectricTestRunner::class)
@GraphicsMode(GraphicsMode.Mode.NATIVE)
@Config(sdk = [34])
class CashTransferScreenTest {
    ...
    private fun viewModel() = CashTransferScreenViewModel(
        mockk(relaxed = true),
        mockk(relaxed = true),
    ...
    fun `renders create form`() {
        setScreen()
        composeRule.onNodeWithText("Cash Transfer (V2)").assertIsDisplayed()
    }
