...
/**
 * Represents the status of a Remittance cash transfer, returned by `getCashTransferStatus`
 * (`GET /remittance/{apiVersion}/cashtransfer/{referenceId}`).
 *
 * Mirrors the [CashTransfer] request payload and adds the outcome fields [financialTransactionId],
 * [status], and [reason]. [status] and [reason] are plain strings here (the endpoint returns a bare
 * status string and an error string), and [payerIdentificationType] reuses [PayerIdentificationType].
 * Every field is optional so partial payloads never break deserialization.
 *
 * @property financialTransactionId The transaction id of the cash transfer from the mobile money manager.
 * @property status The status of the cash transfer (e.g., `PENDING`, `SUCCESSFUL`, `FAILED`).
 * @property reason The error description when the cash transfer did not succeed.
 ...
 * @property externalId Merchant-assigned reference used to correlate the transfer for reconciliation.
 ...
 * @property payerIdentity The identity number of the sending party (payer).
 ...
 * @property payerGender Gender code of the sending party.
 */
@Serializable
data class CashTransferStatus(
