...
class AccountHolderTest {
    ...
    private fun fullAccountHolder() = AccountHolder(
        partyIdType = "msisdn",
    ...
    fun `AccountHolder round-trips when fully populated`() {
        ...
        assertEquals(original, jsonSkipDefaults.decodeFromString<AccountHolder>(jsonSkipDefaults.encodeToString(original)))
        assertEquals(original, jsonWithDefaults.decodeFromString<AccountHolder>(jsonWithDefaults.encodeToString(original)))
    }

    @Test(expected = SerializationException::class)
    fun `AccountHolder decoding an empty object throws`() {
        jsonSkipDefaults.decodeFromString<AccountHolder>("{}")
    }
}
