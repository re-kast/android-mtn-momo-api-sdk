...
class RefundStatusTest {
    ...
    @Test
    fun `RefundStatus maps all fields from JSON`() {
        val raw = """
            {
              "amount": "1500",
              "currency": "EUR",
        ...
        """.trimIndent()
        val result = json.decodeFromString<RefundStatus>(raw)
    ...
    @Test
    fun `RefundStatus decodes an empty object with null fields`() {
        val result = json.decodeFromString<RefundStatus>("{}")
    ...
    @Test
    fun `RefundStatus round-trips when fully populated`() {
        val original = RefundStatus(
            amount = "1500",
            currency = "EUR",
            financialTransactionId = "ftx-1",
            externalId = "ext-1",
        ...
        assertEquals(original, jsonWithDefaults.decodeFromString<RefundStatus>(jsonWithDefaults.encodeToString(original)))
    }
}
