...
class RequestToWithdrawStatusTest {
    ...
    @Test
    fun `RequestToWithdrawStatus maps all fields from JSON`() {
        val raw = """
            {
              "amount": "1500",
              "currency": "EUR",
        ...
        """.trimIndent()
        val result = json.decodeFromString<RequestToWithdrawStatus>(raw)
        ...
        assertEquals(StatusTypes.FAILED, result.status)
    ...
    @Test
    fun `RequestToWithdrawStatus decodes an empty object with null fields`() {
        val result = json.decodeFromString<RequestToWithdrawStatus>("{}")
    ...
    @Test
    fun `RequestToWithdrawStatus round-trips when fully populated`() {
        val original = RequestToWithdrawStatus(
            amount = "1500",
            currency = "EUR",
            financialTransactionId = "ftx-1",
            externalId = "ext-1",
        ...
        assertEquals(original, jsonWithDefaults.decodeFromString<RequestToWithdrawStatus>(jsonWithDefaults.encodeToString(original)))
    }
}
