...
class TransferStatusTest {
    ...
    @Test
    fun `TransferStatus maps all fields from JSON`() {
        val raw = """
            {
              "amount": "1500",
              "currency": "EUR",
        ...
        """.trimIndent()
        val result = json.decodeFromString<TransferStatus>(raw)
    ...
    @Test
    fun `TransferStatus decodes an empty object with null fields`() {
        val result = json.decodeFromString<TransferStatus>("{}")
    ...
    @Test
    fun `TransferStatus round-trips when fully populated`() {
        val original = TransferStatus(
            amount = "1500",
            currency = "EUR",
            financialTransactionId = "ftx-1",
            externalId = "ext-1",
        ...
        assertEquals(original, jsonWithDefaults.decodeFromString<TransferStatus>(jsonWithDefaults.encodeToString(original)))
    }
}
